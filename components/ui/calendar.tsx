...
function Calendar({
  ...
  // Ajuste: defini a variável CSS aqui; mude '2rem' para o valor que preferir (ex: '32px' ou '2rem')
  const cssVar: React.CSSProperties = { ['--cell-size' as any]: '2rem' }

  return (
    <DayPicker
      // passa a variável CSS via style
      style={cssVar}
      showOutsideDays={showOutsideDays}
      className={cn(
        // removi a tentativa inválida de [--cell-size:theme(spacing.8)]
      ...
      classNames={{
        ...
        button_previous: cn(
          ...
          // substitui size-[--cell-size] por w/h com var()
      ...
      components={{
        ...
        WeekNumber: ({ children, ...props }) => {
          return (
            <td {...props}>
              <div className="flex items-center justify-center w-[var(--cell-size)] h-[var(--cell-size)] text-center">
                {children}
              </div>
            </td>
          )
...
function CalendarDayButton({
  ...
  return (
    <Button
      ...
      className={cn(
        // substitui min-w-[--cell-size] por min-w-[var(--cell-size)]
